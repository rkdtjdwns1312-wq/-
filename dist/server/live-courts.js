...
const unpack=row=>row?{...JSON.parse(row.payload),version:row.version,updatedAt:row.updated_at}:{courts:[],queue:[],version:0,updatedAt:null};
...
export async function liveCourts(request,env){
  ...
  try{
    if(request.method==='GET')return json({data:unpack(await env.DB.prepare('SELECT * FROM live_courts WHERE id=1').first())});
    ...
    if(!input||!Number.isInteger(input.version)||input.version<0||!['create','join','leave','end'].includes(input.action))return json({error:'입력 내용을 확인해주세요.'},400);
    if(input.action==='create'&&(!env.EDITOR_KEY||request.headers.get('x-kokkiri-editor')!==env.EDITOR_KEY))return json({error:'코트 생성은 운영진만 할 수 있습니다.'},403);
    ...
    let courts=current.courts,queue=current.queue;
    if(input.action==='create'){
      ...
      if(input.court==='queue'){
        ...
        if(input.action==='join'){
          ...
          if(courts.some(c=>c.state!=='finished'&&c.names.includes(name))||queue.some(g=>g.names.includes(name)))return json({error:'이미 코트 또는 다음 대진에 등록된 이름입니다.'},409);
    ...
    const at=new Date().toISOString(),payload=JSON.stringify({courts,queue});
    const result=row
    ...
    return json({data:{courts,queue,version:current.version+1,updatedAt:at}});
