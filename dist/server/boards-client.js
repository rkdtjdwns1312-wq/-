...
export function client(EDITOR,ADMIN,createScheduleTools,createLiveView,MEMBER=false,createScoreHistory){
  ...
  async function board(kind,token){
    ...
    async function more(){
      ...
      try{const x=await api('/api/posts?kind='+kind+'&offset='+offset);if(token!==routeToken)return;
        ...
        $('postList').insertAdjacentHTML('beforeend',x.items.map(p=>'<a class="post'+((kind==='notice'||kind==='schedule')?' post-notice':'')+'" href="#post/'+encodeURIComponent(p.id)+'"><div>'+(kind==='notice'?'<span class="post-label">공지</span>':kind==='schedule'?'<span class="post-label post-label-alt">대진</span>':'')+'<div class="post-title">'+esc(p.title)+(kind==='schedule'&&p.settledAt?'<span class="settled-tag">'+(p.settlementMode==='unscored'?'미반영마감':'마감')+'</span>':'')+'</div>'+(kind==='schedule'&&p.settledAt&&Array.isArray(p.mvp)&&p.mvp.length?'<div class="mvp-line"><span class="mvp-medal" aria-hidden="true">🥇</span> <span class="mvp-title">MVP '+esc(p.mvp.join(' '))+'</span></div>':'')+((EDITOR||kind!=='notice')?'<small>'+esc(date(p.created_at))+(p.version>1?' · 수정됨':'')+'</small>':'')+'</div></a>').join(''));
  ...
  async function detail(id,token){
    ...
    const endBtn=(EDITOR&&d.kind==='schedule'&&!d.settledAt)?'<div class="end-match-wrap"><button id="endMatch" class="primary end-match">대진 마감</button></div>'+(!allScoredDone?'<div class="end-note">아직 승패를 기록하지 않은 대진이 있어요. <b>'+incompleteRounds.join(', ')+'라운드</b>의 결과를 모두 입력해야 점수를 반영하는 <b>예</b>를 누를 수 있습니다. 지금 마감하려면 <b>미반영마감</b>을 선택하세요.</div>':''):'';
    app.innerHTML=crumb(d.kind)+'<article class="panel detail"><div class="bar"><div>'+(d.kind==='notice'?'<span class="post-label">공지사항</span>':'')+'<h1>'+esc(d.preTitle||d.title)+'</h1>'+((EDITOR||d.kind!=='notice')?'<p class="muted">등록 '+esc(date(d.createdAt))+(d.version>1?' · 수정 '+esc(date(d.updatedAt)):'')+'</p>':'')+(d.settledAt?'<span class="settled-badge">'+(d.settlementMode==='unscored'?'미반영마감 · 시드 미반영':'점수 반영 완료')+'</span>':'')+'</div>'+actions+'</div>'+(d.kind==='notice'?'<div class="notice-body">'+noticeHTML(d.body)+'</div>':'<div class="schedule-meta"><span class="chip">참가 '+d.names.length+'명</span><span class="chip">'+d.courts+'코트</span><span class="chip">'+d.rounds+'라운드</span></div>'+scheduleHTML(d,false,!d.settledAt,roster)+endBtn)+'</article>';
    ...
    if($('endMatch'))$('endMatch').onclick=()=>endMatch(d,token,allScoredDone);
  ...
  async function endMatch(d,token,scoredAllowed=true){
    if(saving||ending||!currentDetail(d.id,token))return;ending=true;
    try{
      const choice=await confirmDialog('정말 이 대진을 마감할까요? 예를 누르면 점수가 반영되고, 시드현황의 순위·점수 변동과 출석·승패 표시가 이 대진의 결과로 바뀝니다. (마감 후에도 되돌릴 수 있어요.)',{settlement:true,scoredAllowed});
      if(!choice)return;
      const mode=choice==='unscored'?'unscored':'scored';
      if(mode==='scored'&&!scoredAllowed){message('승패 결과를 모두 기록한 뒤 점수 반영 마감을 할 수 있어요. 미반영마감은 지금도 가능합니다.',true);return;}
      if(!currentDetail(d.id,token))return;saving=true;message(mode==='unscored'?'점수 반영 없이 마감하는 중…':'점수를 반영하는 중…');
      try{const response=await api('/api/posts/'+encodeURIComponent(d.id)+'/settle',{method:'POST',headers:{'content-type':'application/json','x-kokkiri-editor':EDITOR},body:JSON.stringify({version:d.version,operation:crypto.randomUUID(),mode})}),responseMode=response?.data?.settlementMode==='unscored'?'unscored':'scored';if(Number(response?.data?.version)!==d.version+1)throw Error('최신 대진표를 다시 확인한 뒤 마감을 눌러주세요.');if(responseMode!==mode)throw Error('다른 방식으로 이미 마감되어 최신 대진표를 불러옵니다.');if(!currentDetail(d.id,token))return;message(mode==='unscored'?'미반영마감했어요. 시드현황·점수·출석·승패·MVP는 바뀌지 않습니다.':'점수 반영이 완료되었습니다. 시드현황에 새 순위가 반영됩니다.');routeToken++;detail(d.id,routeToken);window.scrollTo(0,0);}
      catch(e){if(currentDetail(d.id,token)){message(e.message,true);if(/다른 기기|최신|충돌|다른 방식/.test(e.message||''))await refreshDetail(d,token);}}
      finally{saving=false;}
  ...
  async function unsettlePost(d,token){
    ...
    try{
      const unscored=d.settlementMode==='unscored';
      if(!(await confirmDialog(unscored?'이 대진의 미반영마감을 취소할까요? 점수는 바뀌지 않고, 다시 기록·수정할 수 있게 됩니다.':'이 대진의 마감을 취소할까요? 반영됐던 출석·승패 점수가 되돌려지고, 다시 기록·수정할 수 있게 됩니다.')))return;
      if(!currentDetail(d.id,token))return;saving=true;
      try{const response=await api('/api/posts/'+encodeURIComponent(d.id)+'/unsettle',{method:'POST',headers:{'content-type':'application/json','x-kokkiri-editor':EDITOR},body:JSON.stringify({version:d.version})});if(Number(response?.data?.version)!==d.version+1)throw Error('최신 대진표를 다시 확인한 뒤 마감 취소를 눌러주세요.');if(!currentDetail(d.id,token))return;message(unscored?'미반영마감을 취소했어요. 점수는 바뀌지 않았고, 다시 기록할 수 있습니다.':'마감을 취소했어요. 다시 기록할 수 있습니다.');routeToken++;detail(d.id,routeToken);window.scrollTo(0,0);}
  ...
  function confirmDialog(msg,options){options=options||{};return new Promise(resolve=>{let done=false,settlement=options.settlement===true,scoredAllowed=options.scoredAllowed!==false;const onCancel=e=>{e.preventDefault();finish(false);},onClose=()=>finish(false),finish=v=>{if(done)return;done=true;dialog.removeEventListener('cancel',onCancel);dialog.removeEventListener('close',onClose);if(dialog.open)try{dialog.close();}catch(e){}resolve(v);};dialog.innerHTML='<div class="confirm-box"><p class="confirm-msg">'+esc(msg)+'</p>'+(settlement?'<p class="end-note">'+(scoredAllowed?'예는 점수를 반영합니다. 점수 반영 없이 대진만 잠그려면 미반영마감을 선택하세요.':'승패 결과가 모두 기록되지 않아 <b>예</b>는 사용할 수 없습니다. 점수·순위·출석·승패·MVP를 바꾸지 않고 잠그려면 <b>미반영마감</b>을 선택하세요.')+'</p>':'')+'<div class="confirm-actions"><button id="confirmNo">아니오</button><button id="confirmYes" class="primary"'+(settlement&&!scoredAllowed?' disabled':'')+'>예</button>'+(settlement?'<button id="confirmUnscored">미반영마감</button>':'')+'</div></div>';$('confirmYes').onclick=()=>{if(scoredAllowed)finish(settlement?'scored':true);};$('confirmNo').onclick=()=>finish(false);if(settlement)$('confirmUnscored').onclick=()=>finish('unscored');dialog.addEventListener('cancel',onCancel);dialog.addEventListener('close',onClose);dialog.showModal();});}
