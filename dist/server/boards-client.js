export function client(EDITOR,ADMIN,createScheduleTools,createLiveView){
  ...
  function scheduleHTML(d,editing=false,showResults=false){
    ...
    const partnerNote=editing?'<p class="edit-info partner-summary">'+(partnerSummary.duplicateTeams?'팀 파트너 중복 '+partnerSummary.duplicateTeams+'건이 남았습니다. 참가 인원·라운드 수 또는 편성 조건에 따라 중복이 남을 수 있습니다.':'동일·인접 경기의 팀 파트너 중복이 없습니다.')+'</p>':'';
  ...
  async function addBalancedCourt(ri){
    ...
    try{
      ...
      if(r.method!=='random'){
        ...
        match=scheduleTools.balanceFour(four.map(player),r.method||'balanced',previous).map(p=>p.name);
      }
      if(!active()||saving)return;
