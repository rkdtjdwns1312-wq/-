export function client(EDITOR,ADMIN,createScheduleTools){
  ...
  async function endMatch(d){
    if(!(await confirmDialog('정말 이 대진을 마감할까요? 마감하면 출석·승패 점수가 시드현황에 반영됩니다. (마감 후에도 되돌릴 수 있어요.)')))return;
  ...
  async function seeds(token){
    ...
    function render(){
      ...
      if(type==='member'){
        ...
        const rows=list.map(p=>'<tr>'+cb(p.id)+'<td>'+p.rank+'</td><td>'+movement(p)+'</td><td>'+nameHTML(p.name)+'</td><td><span class="seed-badge">'+seedHTML(p.seed||'미정')+'</span></td><td><span class="pts-cell">'+(typeof p.points==='number'?p.points:'-')+'</span>'+pointsMove(p)+'</td><td>'+(p.attendance??0)+'</td><td>'+(p.wins??0)+'</td><td>'+(p.losses??0)+'</td></tr>').join('');
